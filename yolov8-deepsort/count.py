...
if __name__ == '__main__':
    ...
    while True:
        ...
        for item_bbox in list_bboxs:
            ...
            directory = "./directory/" + str(track_id)
            now = datetime.datetime.now()
            generatorname = now.strftime("%H-%M-%S-%MS")
            if not os.path.exists(directory):
                os.makedirs(directory)
            
            print(x1,x2, y1, y2, _, track_id)
            roi = im[y1:y2, x1:x2]
            cv2.imshow("roi",roi)
            filename = directory + "/" + str(track_id)+ str(generatorname) +".jpg"
        ...
        trail_colors = [(255, 255, 255)] * len(object_trails)  # Red color for all trails
