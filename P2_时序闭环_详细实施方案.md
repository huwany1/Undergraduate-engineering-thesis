...
```mermaid
flowchart TD
    subgraph CorePillars["核心防御与韧性保障 (Core Resilience Pillars)"]
        direction TB
        M1["🛡️ 高可用 (High Availability)<br/>· 施密特双滞后防临界震荡<br/>· 最大耗时超时自愈回滚<br/>· 异常/丢帧平滑降级"]
        M2["🎯 高内聚 (High Cohesion)<br/>· 运动学换算/滤波平滑/状态机/计数器职责单一聚焦<br/>· 各子模块接口边界清晰"]
        M3["🔌 低耦合 (Low Coupling)<br/>· 仅消费 P1 中立 DTO<br/>· 完全隔离底层视觉推理与上层提示文案"]
    end

    subgraph EvolutionPerf["效能与演进基石 (Performance & Evolution)"]
        direction TB
        M4["⚡ 高性能 (High Performance)<br/>· 1€ 纯标量因果平滑<br/>· O(1) 无内存冗余分配，微秒级执行"]
        M5["🛠️ 可维护 (Maintainability)<br/>· 状态转移全确定性，无未定义悬空分支<br/>· 100% 覆盖边界的自动化防回归测试矩阵"]
    end

    CorePillars --> P2System["P2 时序闭环架构保障"]
    EvolutionPerf --> P2System
```
