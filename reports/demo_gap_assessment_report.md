...
| **摄像头实时捕捉 (Live Webcam)** | 暂未接入 WebRTC/MediaStream 硬件视频流 | 点击“开启摄像头”，用户在镜头前深蹲，画面实时绘制骨架、实时刷新当前膝角/倾角，并实时完成计数 | 📌 **[TODO 待实现项]**<br>已预留同构 Canvas 渲染管线 |
| **即时多模态反馈** | 仅能在视频播放结束后静态查看文字卡片 | 动作下蹲到及格深度时产生微震动或音效（Web Audio API 提示音），起身判定完成时即时语音提示（如“第 1 次，深度达标”） | 🟢 **P2 体验增强 (规划中)** |

> [!NOTE]
> **关于 [TODO] 摄像头实时捕捉 (Live Webcam) 的实施规划备忘**：
> 1. **前端渲染引擎已 100% 同构就绪**：当前已上线的 `SkeletonRenderer` 纯矢量 Canvas 叠层架构，直接兼容 `<video>` 与 `<canvas>` 流，后续接入 `navigator.mediaDevices.getUserMedia` 时无需重写任何骨骼与角度绘制逻辑；
> 2. **技术路线规划**：
>    - **轻量端侧模式 (首选)**：利用 `@mediapipe/tasks-vision` 的 WASM/WebGL 在浏览器端直接以 30 FPS 推理实时流，零服务端网络传输开销；
>    - **服务端流式模式 (备选)**：通过 WebSocket 将视频帧分块或压缩帧传输至后端 `OnlineAnalysisManager` 进行流式推理，返回时序增量包。

#### 深度原因诊断与已解构短板
在早期版本中，Web Demo 架构本质上是一个“静态报告与预渲染素材分发器”。在当前演进阶段，系统已成功打通“自定义上传 $\to$ 逐帧推理 $\to$ 进度推送 $\to$ 专属持久化”以及“高频 60 FPS 画面矢量骨架绘制 + 实时双角刷新 + 动态累加计数”，彻底破除了“离线播放器”的单一感。现阶段的核心拓展点已完全聚焦至【摄像头硬件实时接入】与【多 Rep 切片下钻】。
...
gantt
    ...
    [TODO] 摄像头硬件实时捕捉与端侧推理接入    :active, p2_0, 2026-09-20, 2d
    多动作 Rep 切片联动交互卡片               :p2_1, after p2_0, 2d
...
### 2. Phase 2：体验与算法跃升（进行中）
- **核心任务**：
  1. **[TODO 核心项] 开启摄像头实时捕捉**：接入浏览器 WebRTC/MediaStream 硬件视频流，利用已就绪的 Canvas 渲染器实现实时镜头前深蹲的即时骨架绘制与计数；
  2. **动作切片下钻**：实现连续动作的 Rep 切片化交互与单次下钻；
