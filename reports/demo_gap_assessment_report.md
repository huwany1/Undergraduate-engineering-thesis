...
1. **YOLOv8 选人与目标跟踪联动演示模块 [TODO 待实施]**：拟将既有 `yolov8-deepsort/demo.py` 封装为 Web 端可选模式（多人/复杂背景选人与 Bounding Box 锁定），用于答辩时呼应开题报告题目；
2. **大模型（LLM）对话交互点 [🟢 已完成 (DELIVERED)]**：**全面接入 DeepSeek API（`deepseek-chat` / `deepseek-reasoner`）实现端云协同**！已落地：
   - 顶部导航栏 API 凭证配置弹窗与一键极速连通性探测（实时测算 RTT 往返延迟毫秒）；
   - 生物力学事实锚定器（Grounded Prompt）：将极限膝屈曲角、前倾角、完成次数与规则原因码注入上下文，严格杜绝大模型幻觉与医疗诊断用语；
   - 玻璃拟态 AI 教练对话交互卡片：提供自然语言个性化评语（带打字机流式呈现）、快捷提问芯片（Quick Prompts）与多轮互动问答；
   - 优雅离线降级兜底：断网、无 Key 或超时 100% 自动回退至本地专家规则模板，全量 190 项测试保持 100% PASS。
