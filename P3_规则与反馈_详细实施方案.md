...
quadrantChart
    ...
    "高内聚 (规则定义/聚合/模板三权分立)": [0.85, 0.88]
    "可维护 (纯配置化规则卡与非医疗词典)": [0.80, 0.82]
    "低耦合 (中立消费P2/对UI渲染完全解耦)": [0.75, 0.65]
    "高可用 (门控拒绝一票否决/自愈兜底)": [0.70, 0.75]
    "高性能 (无分配纯谓词/微秒级内存运算)": [0.90, 0.40]
```
...
mindmap
  root((P3 自动化测试套件))
    周期完整性测试
      正常完成周期准入
      中途放弃 ABORTED
      超速抖动 DISCARDED_TOO_FAST
      超时停滞 DISCARDED_TIMEOUT
    深度代理测试
      深度达标 85度
      轻度浅蹲 102度
      重度浅蹲 120度
